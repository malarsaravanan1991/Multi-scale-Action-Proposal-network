...
def make_temporal_shift(net, n_segment, n_div=8, place='blockres'):
    n_segment_list = [n_segment] * 12 #modified
    ...
    if place == 'block':
            ...
            for i in range(len(net)):
                for block in range(len(net[i].branches)):
                    net[i].branches[block]= make_block_temporal(net[i].branches[block],n_segment_list[count])
                    count +=1
